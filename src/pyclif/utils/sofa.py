...
def convert_key_value_pair_to_sql_clause(
    category_key: str, 
...
def convert_query_dict_to_sql_clause(query_dict: Dict[str, List[str]], category_name: str) -> str:
    """
    Convert a query dictionary to a SQL clause
    """
    return ' OR '.join([convert_key_value_pair_to_sql_clause(k, v, category_name) for k, v in query_dict.items()])

table_name_to_category_name_mapper = {
    "vitals": "vital_category",
    "labs": "lab_category",
    "medication_admin_continuous": "med_category",
    # "patient_assessments": "assessment"
}
...
def lookup_extremal_values_in_long_table(
    ...
    df: pd.DataFrame, 
    table_name: str = acceptable_table_names
) -> pd.DataFrame:
    """
    ...
    SELECT *
    FROM joined
    WHERE {convert_query_dict_to_sql_clause(query_dict, category_name)}
    """
...
def lookup_extremal_vital_values(
    ...
    SELECT *
    FROM joined
    WHERE {convert_query_dict_to_sql_clause(query_dict)}
    """
    qualified = duckdb.sql(query).to_df()
...
def lookup_lab_values(
    hospitalization_ids: List[int], lab_categories: Dict[str, str]
) -> pd.DataFrame:
    """
    Lookup lab values 
    """
