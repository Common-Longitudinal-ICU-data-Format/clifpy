...
| 1     | Creatinine ≤2.0 mg/dL (≤170 μmol/L) OR urine output <0.5 mL/kg/h for 6–12 h                              | DONE: creatinine + UO rate via MIMIC-style trailing self-join. Net UO = output (urine) - input (flush_irrigation_urine). Rate = vol / weight / obs_hours, valid when obs_hours >= 6h. Score 1 if 6h rate < 0.5 AND 12h rate ≥ 0.5. |                |
...
> **Detailed reference**: See `[net_urine_output_rate_computation.md](./net_urine_output_rate_computation.md)` for a worked example with irregular timestamps, explanation of the self-join trailing accumulation, observation time vs. clock span, and design decisions (no proportional splitting, N-1 convention, validity gate).
